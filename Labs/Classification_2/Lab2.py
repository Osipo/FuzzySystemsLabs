...
def estimatePrediction(clf,Xt,Yt,X_test,labels,classnames):
    clf.fit(Xt,Yt)
    Ans = clf.predict(X_test)
    print(classnames)
    print(confusion_matrix(labels,Ans))
...
target_names = X['price_range'].unique()
...
for clf, label in zip([clf0,clf1, clf2, clf3, eclf], ['Decision Tree','Random Forest', 'Extra tree', 'AdaBoost', 'Ensemble']):
    ...
    print('Accurancy_score: '+str(estimatePrediction(clf,X_train,y_train,X_test,y_test,target_names))+' :: '+label)
